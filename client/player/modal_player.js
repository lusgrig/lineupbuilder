...
Template.tModalPlayer.events({
  'click .save-changes': function(evt, tmpl) {
    ...
    var emailPosition = $('.email-position').val();
    var gerseyNumber = $('.gersey-number').val();
    var seasonFeeOwed = $('.season-fee-owed').val();
    var seasonFeePaid = $('.season-fee-paid').val();
    updatePlayer(firstName, gameStatus, fieldPosition, emailPosition, gerseyNumber, seasonFeeOwed, seasonFeePaid);
    $("#modal-id").modal("hide");
    Session.set('sPlayerId', null);
  }
});

var updatePlayer = function(firstName, gameStatus, fieldPosition, emailPosition, gerseyNumber, seasonFeeOwed, seasonFeePaid) {
